...
def main():
    ...
    try:
        ...
        # number of processors required for parallelization
        processor_cnt = 4
        pool = Pool(processes=processor_cnt)
        # list to store the parameters for the parallel function
        parallel_parameters = []
        for path in path_data:
            parallel_parameters.append([path, pattern_data])
        # for each path, detect the best match in patterns using detect_pattern function
        algo_output = pool.map(detect_pattern, parallel_parameters)
